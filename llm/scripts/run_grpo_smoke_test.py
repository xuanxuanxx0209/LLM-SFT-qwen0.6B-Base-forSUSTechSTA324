#!/usr/bin/env python3
"""Run a bounded 30-sample OpenRLHF GRPO smoke test."""
...
def select_shortest_prompt_records(tokenizer, records: list[dict], limit: int) -> list[dict]:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--chat-template-file",
        default=str(SCRIPT_DIR / "grpo_async_pipeline" / "templates" / "qwen3_06b_base_eot_chat_template.jinja"),
    )
    parser.add_argument(
        "--reward-script",
        default=str(SCRIPT_DIR / "grpo_async_pipeline" / "math_exact_match_reward.py"),
    )
    parser.add_argument("--sample-count", type=int, default=30)
    parser.add_argument("--max-len", type=int, default=1024)
    parser.add_argument("--prompt-max-len", type=int, default=512)
    parser.add_argument("--generate-max-len", type=int, default=256)
...
def run_command(cmd: list[str], log_path: Path) -> None:
    ...
    if return_code != 0:
        raise RuntimeError(f"GRPO smoke test failed with return code {return_code}. See {log_path}")
...
def main() -> None:
    ...
    run_root = Path(args.run_root).resolve() if args.run_root else result_root / f"grpo_smoke_{utc_stamp()}"
    run_root.mkdir(parents=True, exist_ok=True)

    tokenizer = AutoTokenizer.from_pretrained(str(model_path), trust_remote_code=True)
    train_records = select_shortest_prompt_records(tokenizer, load_jsonl(Path(args.train_dataset)), args.sample_count)
    valid_records = select_shortest_valid_records(tokenizer, load_jsonl(Path(args.validation_input)), args.sample_count)

    train_subset = smoke_data_root / "grpo_train_30.jsonl"
    valid_subset = smoke_data_root / "grpo_valid_30.jsonl"
    ...
    cmd = [
        python_bin,
        str(SCRIPT_DIR / "grpo_async_pipeline" / "run_openrlhf_grpo_train_eval_best.py"),
        "--model-path",
        str(model_path),
        "--train-dataset",
        str(train_subset),
        "--reward-script",
        str(Path(args.reward_script).resolve()),
        "--validation-input",
        str(valid_subset),
        "--chat-template-file",
    ...
    if not summary_path.exists():
        raise FileNotFoundError(f"Missing GRPO evaluation summary: {summary_path}")
