#!/usr/bin/env python3
"""Run validation-set generation and scoring for one GRPO checkpoint."""
...
SFT_PIPELINE_DIR = PROJECT_ROOT / "scripts" / "sft_async_pipeline"
...
def main() -> None:
    ...
    vllm_cmd = [
        sys.executable,
        str(SFT_PIPELINE_DIR / "run_vllm_valid_single_gpu.py"),
        "--model-path",
        str(model_path),
        "--tokenizer-path",
        str(tokenizer_path),
    ...
    hf_cmd = [
        sys.executable,
        str(SFT_PIPELINE_DIR / "run_hf_valid_single_gpu.py"),
        "--model-path",
        str(model_path),
        "--tokenizer-path",
        str(tokenizer_path),
